...
export default function ChatRoom({ navigation }) {
      ...
      const unsubscribe = firestore()
        ...
        .onSnapshot((querySnapshot) => {
          ...
          const filtered = threads
            .filter((e) => {
              return (
                (e?.createdBy == profile?.username ||
                  e?.otherUser == profile?.username) &&
                e.latestMessage.text.length > 1
              );
  ...
  const renderItem = ({ item }) => {
    console.log(
      "IMAGEURL",
      API_URL + "/static/uploads/users/" + item?.otherUserUID + "/IMG.jpg"
    );
    return (
      <TouchableOpacity
        onPress={() => {
        ...
        <View style={styles.row}>
          ...
          <View style={styles.content}>
            ...
            <Text style={styles.contentText}>
              {item?.latestMessage?.createdBy === profile?.username
                ? "You"
                : item?.latestMessage?.createdBy}{" "}
