...
const PostScreen = ({ navigation, route }) => {
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <View style={{ width: '100%', backgroundColor: '#fff', padding: 10, borderBottomColor: '#aaa', borderBottomWidth: 0.2 }}>
                <Text style={{ position: 'absolute', right: 0, fontSize: 10, color: '#888', top: 10, right: 10 }}>{msToTime(UTCseconds - item?.created_at?.$date)}</Text>
    ...
    return (
        <View style={{ height: '100%', width: '100%', backgroundColor: '#fff', flexDirection: 'column', justifyContent: 'flex-start', flex: 2 }}>
            ...
            <View style={{ position: 'absolute', width: '100%', bottom: 0, height: 60, backgroundColor: '#fff' }}>
                    ...
                    <Input
                        ...
                        rightIcon={
                            <MaterialIcons onPress={async () => {
                                const res = await postsAPI.writeComment(profileInfo?._id?.$oid, comment, route?.params?.item?._id?.$oid)
                                dispatch(getCommentsThunk(route?.params?.item?._id?.$oid))
                                console.log("RES", res)
                                setComment('');
