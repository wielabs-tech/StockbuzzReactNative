...
export default GroupPage = ({ navigation, route }) => {
  ...
  const isParticipant = myRooms?.find(element => element?._id?.$oid == route.params._id?.$oid)
  ...
  navigation.setOptions({
    ...
    headerLeft: () => (
      <Icon onPress={() => {
        navigation.goBack();
      }} name="arrow-back" size={24} style={{ marginLeft: 10 }} />
    )
  })
  ...
  return (
    <SafeAreaView>
      <View style={styles.container}>
          ...
          <View style={{ flexDirection: 'row' }}>
            {
              groupInfo?.image ?
                ...
                /> :
                <View style={{ width: 64, height: 64, borderRadius: 64, backgroundColor: '#aaa', justifyContent: 'center' }}>
                  <MaterialIcons name="group" size={36} color="#fff" style={{ alignSelf: 'center' }} />
                </View>
            }
            <View style={{ alignSelf: 'center', marginLeft: 5 }}>
              <Text style={{ fontFamily: 'SecularOne-Regular', fontSize: 16 }}>{groupInfo?.title}</Text>
              <Text style={{ fontSize: 12, }}>since {dateFormat(date, "dS mmmm yyyy")}</Text>
            </View>
          </View>
          <View style={{ alignSelf: 'center' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
              <View style={{ flexDirection: 'row' }}>
                ...
                <Text style={{ marginLeft: 2, color: '#000', fontSize: 8, alignSelf: 'flex-end' }}>People</Text>
              </View>
              <Text
        ...
        </View>
        {
          !isParticipant ? (
            <View style={{ marginTop: 10 }}>
              <TouchableOpacity
                onPress={() => {
                  dispatch(joinGroupThunk(groupInfo?._id?.$oid, profile?._id?.$oid))
                }}
                style={{
                  marginHorizontal: 20,
                  alignContent: 'center',
                ...
                <Text style={{ color: '#fff', alignSelf: 'center' }}>Join group</Text>
              </TouchableOpacity>
            </View>
          ) : (
            (
              <View style={{ marginTop: 10 }}>
                <TouchableOpacity
                  ...
                  <Text style={{ color: '#fff', alignSelf: 'center' }}>Exit group</Text>
                </TouchableOpacity>
              </View>
            )
          )
        ...
        <Text style={{ marginTop: 20, color: '#5b72ff', marginLeft: 10 }}>Posts</Text>
        <View style={{ height: 2, backgroundColor: '#aaa', marginTop: 10 }}></View>
        <FlatList
          data={roomPosts}
      ...
      <TouchableOpacity style={{ 
        ...
        onPress={() => {
          console.log("PRESSED")
          navigation.push('createPost', { groupPost: true, groupId: groupInfo?._id?.$oid })
        }}
      >
        <LinearGradient
