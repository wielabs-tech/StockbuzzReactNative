...
export const postLoginThunk = (email, password) => async (dispatch) => {
  try {
    await messaging().registerDeviceForRemoteMessages();
    const token = await messaging().getToken();
    const response = await loginAPI.login(email, password, token);
...
export const signUpThunk =
  (email, password, username, fullName, mobile, image, login_type, token) =>
  async (dispatch) => {
    try {
...
export const signUpThunkSocial =
  (email, password, username, fullName, mobile, image, login_type, uid) =>
  async (dispatch) => {
    try {
...
export const loginWithSocial = (uid) => async (dispatch) => {
  try {
    ...
    if (response?.status == 200) {
      console.log("RESSS", response?.data?._id.$oid);
      await dispatch(getProfileInfoThunk(response?.data?._id.$oid));
      dispatch(login(response?.data));
    }
  } catch (e) {
