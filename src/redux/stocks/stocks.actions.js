...
export const getWatchlistDataUsingAPI = (array) => async (dispatch) => {
  array.forEach(async (element) => {
    const res = await stocksAPI.getStockInfo(element);
...
export const getCryptoSuggestionsThunk =
  (startText, cryptos) => async (dispatch) => {
    dispatch({
      type: CRYPTO_SUGGESTIONS,
      payload: cryptos
        ?.filter((country) => country.slug.startsWith(startText))
        .map((e) => {
